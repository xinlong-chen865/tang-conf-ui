<script lang="ts">
  ...
  export let onSuccess: (responses: Record<string, any>) => any | undefined =
    undefined;
  export let onError: (responses: Record<string, any>) => any | undefined =
    undefined;
  ...
  const updateValue = (key: string, type: string, event: Event) => {
    if (type === "number")
      responses[key] = Number((event.target as HTMLInputElement).value);
  ...
  const downloadFile = async (downloadFunction?: Function) => {
    if (typeof downloadFunction !== "function" || state === "downloading")
      return;
    state = "downloading";
              ...
              <div class="flex items-start mt-2 mr-4">
                <div class="flex items-center h-5">
                  <input
                    ...
                    on:input={(event) =>
                      updateValue(item.name, item.type, event)}
                    name={item.name}
                    {value}
